...
namespace :one_offs do
  ...
  task backfill_teacher_catchment_iso_country_code: :environment do
    ...
    NPQApplication.where.not(teacher_catchment: [nil, "another"]).find_each do |npq_application|
      next unless npq_application.in_uk_catchment_area?
    ...
    NPQApplication.where.not(teacher_catchment_country: nil).where(teacher_catchment: [nil, "another"]).find_each do |npq_application|
