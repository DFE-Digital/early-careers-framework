...
namespace :payment_calculation do
  ...
  task breakdown: :environment do
    ...
    total_participants = (ARGV[2] || ParticipantEventAggregator.call(cpd_lead_provider: cpd_lead_provider, scope: :active_for_lead_provider)).to_i
    uplift_participants = (ARGV[3] || ParticipantEventAggregator.call(cpd_lead_provider: cpd_lead_provider, scope: :active_uplift_for_lead_provider)).to_i
    total_ects = (ARGV[2].present? ? ARGV[2].to_i / 2 : ParticipantEventAggregator.call(cpd_lead_provider: cpd_lead_provider, scope: :active_ects_for_lead_provider))
    total_mentors = (ARGV[2].present? ? ARGV[2].to_i - ARGV[2].to_i / 2 : ParticipantEventAggregator.call(cpd_lead_provider: cpd_lead_provider, scope: :active_mentors_for_lead_provider))
  ...
  task csv: :environment do
    ...
    CSV.open(filename, "wb") do |csv|
      ...
      lead_providers.each do |lead_provider|
        ...
        total_participants = ParticipantEventAggregator.call(cpd_lead_provider: lead_provider.cpd_lead_provider, scope: :active_for_lead_provider).to_i
        uplift_participants = ParticipantEventAggregator.call(cpd_lead_provider: lead_provider.cpd_lead_provider, scope: :active_uplift_for_lead_provider).to_i
        total_ects = ParticipantEventAggregator.call(cpd_lead_provider: lead_provider.cpd_lead_provider, scope: :active_ects_for_lead_provider)
        total_mentors = ParticipantEventAggregator.call(cpd_lead_provider: lead_provider.cpd_lead_provider, scope: :active_mentors_for_lead_provider)
