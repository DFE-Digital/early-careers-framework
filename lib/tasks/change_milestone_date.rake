...
namespace :change_milestone_date do
  ...
  task :validate, %i[schedule_identifier start_year milestone_number new_start_date new_milestone_date new_payment_date] => :environment do |_task, args|
    args = args.to_h
    change = ChangeMilestoneDate.new(args)

    if change.valid?
      puts "Milestone date can be changed to #{args.slice(:new_start_date, :new_milestone_date, :new_payment_date)}"
  ...
  task :execute, %i[schedule_identifier start_year milestone_number new_start_date new_milestone_date new_payment_date] => :environment do |_task, args|
