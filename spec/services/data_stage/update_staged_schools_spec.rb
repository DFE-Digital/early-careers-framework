...
RSpec.describe DataStage::UpdateStagedSchools do
  ...
  describe ".call" do
    it "imports each school_data_file row as a DataStage::School" do
      expect { service.call(**files) }.to change { DataStage::School.count }.by 6
    ...
    it "ensures the local authority is created" do
      expect { service.call(**files) }.to change { LocalAuthority.count }.by 6
    end

    it "ensures the local authority district is created" do
      expect { service.call(**files) }.to change { LocalAuthorityDistrict.count }.by 6
    end
