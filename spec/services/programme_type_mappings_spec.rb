...
RSpec.describe ProgrammeTypeMappings do
  ...
  describe "mappings" do
    ...
    describe ".withdrawl_reason" do
      subject { described_class.withdrawl_reason(reason:) }

      context "when mappings are enabled" do
        let(:mappings_enabled) { true }

        context "when reason is 'switched-to-school-led'" do
          let(:reason) { "switched-to-school-led" }

          it { is_expected.to eq("school-left-fip") }
        end

        context "when reason is not 'switched-to-school-led'" do
          let(:reason) { "other" }
    ...
    describe ".training_programme" do
      ...
      context "when mappings are enabled" do
        ...
        context "when training_programme is not 'full_induction_programme' or 'school_funded_fip'" do
