...
RSpec.describe SetSchoolLocalAuthority do
  ...
  describe ".call" do
    it "creates a school local authority association record" do
      expect {
        service.call(school:, la_code:, start_year:)
    ...
    context "when the local authority is already linked" do
      before do
        SchoolLocalAuthority.create!(school:, local_authority:, start_year: 2020)
      end
    ...
    context "when a school local authority record exists" do
      ...
      before do
        SchoolLocalAuthority.create!(school:, local_authority: old_local_authority, start_year: 2020)
      end
    ...
    context "when no matching local authority exists" do
      ...
      it "raises an exception" do
        expect {
          service.call(school:, la_code: bad_la_code, start_year:)
