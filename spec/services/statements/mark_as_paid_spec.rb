...
RSpec.describe Statements::MarkAsPaid do
  ...
  let(:eligible_statement)            { create(:npq_statement, cpd_lead_provider:, cohort:, deadline_date: cohort.academic_year_start_date + 3.months) }
  let(:statement)                     { create(:npq_statement, cpd_lead_provider:, cohort:, deadline_date: cohort.academic_year_start_date + 2.months) }
