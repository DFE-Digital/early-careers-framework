...
RSpec.describe TrainingRecordState do
  ...
  let(:induction_record) do
    ...
    query = query.joins(:school_cohort).where(school_cohort: { school: current_school }) if current_school.present?
    query.latest
  end
  ...
  describe "optimisations to avoid additional queries" do
    context "when the induction records respond to transient_latest_request_for_details_status" do
      let!(:participant_profile) { scenarios.ect_on_fip_details_request_submitted.participant_profile }

      it "reads this attribute instead of querying the emails table" do
        def induction_record.transient_latest_request_for_details_status
          "submtited"
        end

        expect(Email).not_to receive(:associated_with)

        determined_state.record_state
      end
    end

    context "when the induction records respond to transient_mentees" do
      let!(:participant_profile) { scenarios.mentor_on_fip_no_validation.participant_profile }

      it "reads this attribute instead of querying the emails table" do
        def induction_record.transient_mentees
          true
        end

        expect(InductionRecord).not_to receive(:where)

        determined_state.record_state
      end
    end

    context "when the induction records respond to transient_current_mentees" do
      let!(:participant_profile) { scenarios.mentor_on_fip_no_validation.participant_profile }

      it "reads this attribute instead of querying the emails table" do
        def induction_record.transient_current_mentees
          true
        end

        expect(InductionRecord).not_to receive(:current)
