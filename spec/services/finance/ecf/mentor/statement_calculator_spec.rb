...
RSpec.describe Finance::ECF::Mentor::StatementCalculator, mid_cohort: true do
  ...
  describe "#clawed_back_declarations" do
    ...
    it "returns only clawed back mentor declarations" do
      expect(subject.clawed_back_declarations.size).to eql(5)
      expect(subject.clawed_back_declarations.pluck(:state, :type).uniq.flatten).to eql(["clawed_back", "ParticipantDeclaration::Mentor"])
    end
  end
