...
RSpec.describe AdditionalEmailImporter do
  ...
  describe "#run" do
    ...
    it "adds multiple emails to a single school" do
      ...
      expect(created_emails.map(&:sit_email_address)).to match_array %w[head@example.com admin@example.com]
    end

    it "strips slashes from emails" do
      created_email = School.find_by(urn: 100_001).additional_school_emails.first

      expect(created_email.sit_email_address).to eql "info@example.com"
    end

    it "does not create emails with banned words" do
      expect(AdditionalSchoolEmail.where(sit_email_address: "covid-info@example.com").count).to eql 0
      expect(AdditionalSchoolEmail.where(sit_email_address: "admissions@example.com").count).to eql 0
    end
  end
end
