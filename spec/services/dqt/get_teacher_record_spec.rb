...
RSpec.describe DQT::GetTeacherRecord do
  describe "#call" do
    ...
    context "when date_of_birth is provided" do
      it "returns the participants DQT record" do
        expect_any_instance_of(DQT::V3::Client).to receive(:get_record).with(trn:, date_of_birth:).once.and_return(valid_record)
    ...
    context "when nino is provided" do
      it "returns the participants DQT record" do
        expect_any_instance_of(DQT::V1::Client).to receive(:get_record)
