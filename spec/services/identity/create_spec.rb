  ...
  describe ".call" do
    ...
    context "when the user has existing participant_profiles" do
      context "when no existing identity already exists" do
        ...
        it "adds the profiles to the new identity" do
          identity = service.call(user:)
          expect(identity.participant_profiles).to match_array [mentor_profile, ect_profile]
        end
      end
    end
    ...
    context "when used to create an additional sign-in account" do
      ...
      context "when the user has existing profiles" do
        ...
        let!(:ect_profile) { create(:ect_participant_profile, teacher_profile:, participant_identity: identity) }
