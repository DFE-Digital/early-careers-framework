...
RSpec.describe CocSetParticipantCategories, with_feature_flags: { change_of_circumstances: "active" } do
  describe "#run" do
    ...
    let(:sffip_programme) { create(:induction_programme, :fip, school_cohort:) }
    # FIP
    ...
    # School Funded FIP
    let(:sffip_eligible_ect) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_eligible_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_ineligible_ect) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_ineligible_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_contacted_for_info_ect) { create(:ect_participant_profile, :email_sent, request_for_details_sent_at: 5.days.ago, school_cohort:) }
    let(:sffip_contacted_for_info_mentor) { create(:mentor_participant_profile, :email_sent, request_for_details_sent_at: 5.days.ago, school_cohort:) }
    let(:sffip_ero_ect) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_ero_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_details_being_checked_ect) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_details_being_checked_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_primary_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, :primary_profile, school_cohort:) }
    let(:sffip_secondary_mentor) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, :secondary_profile, school_cohort:) }
    let(:sffip_withdrawn_ect) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, training_status: "withdrawn", school_cohort:) }
    let(:sffip_transferring_in_participant) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_transferring_out_participant) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_transferred_participant) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_transferred_withdrawn_participant) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_ect_no_qts) { create(:ect_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    let(:sffip_mentor_no_qts) { create(:mentor_participant_profile, :ecf_participant_eligibility, :ecf_participant_validation_data, school_cohort:) }
    ...
    let(:sffip_participants) do
      [
        sffip_eligible_ect,
        sffip_eligible_mentor,
        sffip_ineligible_ect,
        sffip_ineligible_mentor,
        sffip_contacted_for_info_ect,
        sffip_contacted_for_info_mentor,
        sffip_ero_ect,
        sffip_ero_mentor,
        sffip_details_being_checked_ect,
        sffip_details_being_checked_mentor,
        sffip_primary_mentor,
        sffip_secondary_mentor,
        sffip_withdrawn_ect,
        sffip_transferring_in_participant,
        sffip_transferring_out_participant,
        sffip_transferred_participant,
        sffip_transferred_withdrawn_participant,
        sffip_ect_no_qts,
        sffip_mentor_no_qts,
      ]
    end
    ...
    context "School with SFFIP default" do
      let(:induction_coordinator) { create(:induction_coordinator_profile, schools: [school_cohort.school]) }

      before do
        setup_sffip_participants
      end

      # NOTE: all categories under one spec as otherwise very slow
      it "returns induction_records in correct category" do
        # eligible
        expect(@ect_categories.eligible).to match_array([sffip_eligible_ect, sffip_ero_ect].map(&:current_induction_record))
        expect(@mentor_categories.eligible).to match_array([sffip_eligible_mentor, sffip_ero_mentor, sffip_primary_mentor, sffip_secondary_mentor].map(&:current_induction_record))

        # ineligible
        expect(@ect_categories.ineligible).to match_array(sffip_ineligible_ect.current_induction_record)
        expect(@mentor_categories.ineligible).to match_array(sffip_ineligible_mentor.current_induction_record)

        # contacted_for_info
        expect(@ect_categories.contacted_for_info).to match_array(sffip_contacted_for_info_ect.current_induction_record)
        expect(@mentor_categories.contacted_for_info).to match_array(sffip_contacted_for_info_mentor.current_induction_record)

        # details_being_checked
        expect(@ect_categories.details_being_checked).to match_array(sffip_details_being_checked_ect.current_induction_record)
        expect(@mentor_categories.details_being_checked).to match_array(sffip_details_being_checked_mentor.current_induction_record)

        # withdrawn
        expect(@ect_categories.withdrawn).to match_array(sffip_withdrawn_ect.current_induction_record)

        # transferring_in
        expect(@ect_categories.transferring_in).to match_array(sffip_transferring_in_participant.induction_records.latest)

        # transferring_out
        expect(@ect_categories.transferring_out).to match_array(sffip_transferring_out_participant.induction_records.latest)

        # transferred
        expect(@ect_categories.transferred).to match_array([sffip_transferred_participant, sffip_transferred_withdrawn_participant].map { |profile| profile.induction_records.latest })

        # no_qts
        expect(@ect_categories.no_qts_participants).to match_array(sffip_ect_no_qts.current_induction_record)
        expect(@mentor_categories.no_qts_participants).to match_array(sffip_mentor_no_qts.current_induction_record)
      end
    end

    context "FIP, CIP and SFFIP induction programmes" do
      let(:induction_coordinator) { create(:induction_coordinator_profile, schools: [school_cohort.school]) }
      ...
      it "returns participants in correct category" do
        # eligible
        expect(@ect_categories.eligible).to match_array([fip_eligible_ect, fip_ero_ect, cip_eligible_ect, cip_ineligible_ect, cip_ero_ect, cip_details_being_checked_ect, cip_ect_no_qts, sffip_eligible_ect, sffip_ero_ect].map(&:current_induction_record))
        expect(@mentor_categories.eligible).to match_array([fip_eligible_mentor, fip_ero_mentor, fip_primary_mentor, fip_secondary_mentor, cip_eligible_mentor, cip_ineligible_mentor, cip_ero_mentor, cip_details_being_checked_mentor, cip_primary_mentor, cip_secondary_mentor, cip_mentor_no_qts, sffip_eligible_mentor, sffip_ero_mentor, sffip_primary_mentor, sffip_secondary_mentor].map(&:current_induction_record))

        # ineligible
        expect(@ect_categories.ineligible).to match_array([fip_ineligible_ect, sffip_ineligible_ect].map(&:current_induction_record))
        expect(@mentor_categories.ineligible).to match_array([fip_ineligible_mentor, sffip_ineligible_mentor].map(&:current_induction_record))

        # contacted_for_info
        expect(@ect_categories.contacted_for_info).to match_array([fip_contacted_for_info_ect, cip_contacted_for_info_ect, sffip_contacted_for_info_ect].map(&:current_induction_record))
        expect(@mentor_categories.contacted_for_info).to match_array([fip_contacted_for_info_mentor, cip_contacted_for_info_mentor, sffip_contacted_for_info_mentor].map(&:current_induction_record))

        # details_being_checked
        expect(@ect_categories.details_being_checked).to match_array([fip_details_being_checked_ect, sffip_details_being_checked_ect].map(&:current_induction_record))
        expect(@mentor_categories.details_being_checked).to match_array([fip_details_being_checked_mentor, sffip_details_being_checked_mentor].map(&:current_induction_record))

        # withdrawn
        expect(@ect_categories.withdrawn).to match_array([fip_withdrawn_ect, cip_withdrawn_ect, sffip_withdrawn_ect].map(&:current_induction_record))
        ...
        # transferring_out
        expect(@ect_categories.transferring_out).to match_array([fip_transferring_out_participant, cip_transferring_out_participant, sffip_transferring_out_participant].map { |profile| profile.induction_records.latest })

        # transferred
        expect(@ect_categories.transferred).to match_array([fip_transferred_participant, fip_transferred_withdrawn_participant, cip_transferred_participant, cip_transferred_withdrawn_participant, sffip_transferred_participant, sffip_transferred_withdrawn_participant].map { |profile| profile.induction_records.latest })

        # no_qts
        expect(@ect_categories.no_qts_participants).to match_array([fip_ect_no_qts, sffip_ect_no_qts].map(&:current_induction_record))
        expect(@mentor_categories.no_qts_participants).to match_array([fip_mentor_no_qts, sffip_mentor_no_qts].map(&:current_induction_record))
      end
    end
  ...
  def setup_sffip_participants
    school_cohort.update!(induction_programme_choice: :full_induction_programme, default_induction_programme: sffip_programme)
    sffip_participants.each do |profile|
      Induction::Enrol.call(participant_profile: profile, induction_programme: sffip_programme)
    end
    sffip_transferring_in_participant.induction_records.first.update!(start_date: 2.months.from_now, school_transfer: true)
    sffip_transferring_out_participant.induction_records.first.update!(school_transfer: true, induction_status: :leaving, end_date: 1.month.from_now)
    sffip_transferred_participant.induction_records.first.leaving!(1.month.ago)
    sffip_transferred_withdrawn_participant.induction_records.first.leaving!(1.month.ago)
    sffip_transferred_withdrawn_participant.induction_records.first.training_status_withdrawn!
    sffip_withdrawn_ect.induction_records.first.training_status_withdrawn!

    sffip_ineligible_ect.ecf_participant_eligibility.update!(status: "ineligible", reason: "active_flags")
    sffip_ineligible_mentor.ecf_participant_eligibility.update!(status: "ineligible", reason: "active_flags")
    sffip_ero_ect.ecf_participant_eligibility.update!(status: "ineligible", reason: "previous_participation")
    sffip_ero_mentor.ecf_participant_eligibility.update!(status: "ineligible", reason: "previous_participation")
    sffip_details_being_checked_ect.ecf_participant_eligibility.update!(status: "manual_check", reason: "different_trn")
    sffip_details_being_checked_mentor.ecf_participant_eligibility.update!(status: "manual_check", reason: "different_trn")
    sffip_ect_no_qts.ecf_participant_eligibility.update!(status: "manual_check", reason: "no_qts")
    sffip_mentor_no_qts.ecf_participant_eligibility.update!(status: "manual_check", reason: "no_qts")

    [sffip_primary_mentor, sffip_secondary_mentor].each do |profile|
      profile.ecf_participant_eligibility.determine_status
      profile.ecf_participant_eligibility.save!
    end
