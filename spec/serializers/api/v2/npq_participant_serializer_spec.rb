      ...
      describe "serialization" do
        ...
        describe "npq_enrollments" do
          ...
          it "returns expected data", :aggregate_failures do
            ...
            expect(result[:data][:attributes][:npq_enrollments][0][:course_identifier]).to eql(profile.npq_course.identifier)
            expect(result[:data][:attributes][:npq_enrollments][0][:schedule_identifier]).to eql(profile.schedule.schedule_identifier)
            expect(result[:data][:attributes][:npq_enrollments][0][:cohort]).to eql(profile.schedule.cohort.start_year.to_s)
            expect(result[:data][:attributes][:npq_enrollments][0][:npq_application_id]).to eql(profile.npq_application.id)
          ...
          context "when there are multiple providers involved" do
            ...
            it "only includes enrollments from the querying provider" do
              ...
              expect(result[:data][:attributes][:npq_enrollments].size).to be(1)
            end
          end
        end
