...
RSpec.describe Finance::BandingTracker::ParticipantPerDeclarationTypePerBandAggregator do
  ...
  let(:participants_per_declaration_state) do
    { "started" => 186, "retained-1" => 20, "retained-3" => 8, "retained-2" => 13, "completed" => 2, "retained-4" => 6 }
  end
  ...
  it "aggregates participants per band" do
    ...
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "started", band: bands[1])).to eq(40)
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "started", band: bands[2])).to eq(60)
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "started", band: bands[3])).to eq(65)
    ...
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "retained-4", band: bands[2])).to eq(0)
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "retained-4", band: bands[3])).to eq(0)

    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "completed", band: bands[0])).to eq(2)
    ...
    expect(aggregator.participants_for_declaration_type_in_band(declaration_type: "completed", band: bands[3])).to eq(0)
  end
end
