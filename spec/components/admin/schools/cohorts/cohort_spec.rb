...
RSpec.describe Admin::Schools::Cohorts::Cohort, type: :view_component do
  ...
  context "without school cohort" do
    ...
    it { is_expected.to have_rendered_component(Admin::Schools::Cohorts::OtherInfo).with(cohort: cohort, school_cohort: nil) }
  end

  context "with CIP school cohort" do
    let(:school_cohort) { FactoryBot.build :school_cohort, :cip }

    it { is_expected.to have_rendered_component(Admin::Schools::Cohorts::CipInfo).with(school_cohort: school_cohort) }
  end

  context "with FIP school cohort" do
    let(:school_cohort) { FactoryBot.build :school_cohort, :fip }

    it { is_expected.to have_rendered_component(Admin::Schools::Cohorts::FipInfo).with(school_cohort: school_cohort) }
  end
  ...
  context "with school cohort that is neither CIP nor FIP" do
    ...
    it { is_expected.to have_rendered_component(Admin::Schools::Cohorts::OtherInfo).with(cohort: cohort, school_cohort: school_cohort) }
  end
end
