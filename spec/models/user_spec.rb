...
RSpec.describe User, type: :model do
  ...
  context "when archiving users" do
    ...
    describe "bulk archiving process" do
      ...
      describe ".select_users_to_archive" do
        let(:users_to_delete) { User.select_users_to_archive }
      ...
      describe ".dedup_users" do
        it "performs deduplication (archiving) users" do
          expect { User.dedup_users! }.to change { deletable_user.reload.email }.from("user@example.org").to("user#{deletable_user.id}@example.org")
        end
      end
    end
  end
