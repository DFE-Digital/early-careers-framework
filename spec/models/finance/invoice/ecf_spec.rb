...
RSpec.describe Finance::Invoice::ECF do
  before do
  ...
  describe "#current" do
    it "returns the milestone including 'ECF' where the interval range includes today" do
      expect(Finance::Invoice::ECF.find_current_milestone.name).to eq "ECF ret-1"
    end
  end

  describe "#payable" do
    it "returns the milestone including 'ECF' where the interval range is most recent" do
      expect(Finance::Invoice::ECF.find_payable_milestone.name).to eq "ECF start"
    end
  end
end
