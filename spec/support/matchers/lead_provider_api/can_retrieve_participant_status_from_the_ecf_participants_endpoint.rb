...
module Support
  module CanRetrieveParticipantStatusFromTheEcfParticipantsEndpoint
    ...
    RSpec::Matchers.define :be_able_to_retrieve_the_status_of_the_participant_from_the_ecf_participants_endpoint do |participant_name, status, options = {}|
      ...
      failure_message do |lead_provider_name|
        "'#{lead_provider_name}' Should have been able to retrieve the \"status\" of \"#{status}\" for \"#{participant_name}\" within\n===\n#{@text}\n==="
      end

      failure_message_when_negated do |lead_provider_name|
        "'#{lead_provider_name}' Should not have been able to retrieve the \"status\" of \"#{status}\" for \"#{participant_name}\" within\n===\n#{@text}\n==="
      end

      description do
        "be able to retrieve the \"status\" of \"#{status}\" for \"#{participant_name}\" from the ecf participants endpoint"
      end
    end
  end
end
