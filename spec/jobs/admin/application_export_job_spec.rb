...
RSpec.describe Admin::ApplicationExportJob do
  describe "#perform" do
    subject { described_class.new.perform(npq_application_export) }

    let(:npq_application_export) do
      NPQApplicationExport.create(start_date:, end_date:, user: create(:user))
    end
    ...
    before do
      ...
      ENV.stub(:[]).with("GOOGLE_DRIVE_NPQ_UPLOAD_FOLDER_ID").and_return(parent_folder_id)
    end
