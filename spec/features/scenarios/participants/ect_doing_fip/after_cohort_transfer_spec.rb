...
RSpec.feature "ECT doing FIP: after cohort transfer", type: :feature do
  let!(:participant_details) do
    NewSeeds::Scenarios::Participants::Ects::EctAfterCohortTransfer
      .new(school_cohort:, new_school_cohort:, full_name: participant_full_name)
  ...
  let(:previous_cohort) do
    Cohort.find_by(start_year: previous_start_year)
          .tap { |previous_cohort| allow(Cohort).to receive(:previous).and_return(previous_cohort) }
  end
  ...
  %w[active inactive].each do |flag_state|
    context "when programme type changes for 2025 are #{flag_state}", with_feature_flags: { programme_type_changes_2025: flag_state } do
      let(:programme_name) do
        # FIXME: LPDOB have used "Provider led" instead of "Provider-led" so using that until fixed
