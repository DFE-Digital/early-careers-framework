...
RSpec.feature "ECT doing CIP: no validation", type: :feature, early_in_cohort: true do
  ...
  let(:participant_full_name) { "ECT doing CIP: no validation" }
  ...
  %w[active inactive].each do |flag_state|
    context "when programme type changes for 2025 are #{flag_state}", with_feature_flags: { programme_type_changes_2025: flag_state } do
      ...
      scenario "The current school induction tutor can locate a record for the ECT" do
        ...
        expect(participant_details).to have_materials_supplier(cip_material_provider) unless FeatureFlag.active?(:programme_type_changes_2025)
      end
