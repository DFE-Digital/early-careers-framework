...
RSpec.feature "FIP to FIP with same provider - Onboarding a withdrawn participant", type: :feature, end_to_end_scenario: true do
  ...
  CSV.parse(File.read(fixture_data_path), headers: true).each_with_index do |fixture_data, index|
    ...
    context given_context(scenario) do
      ...
      context when_context(scenario) do
        ...
        include_examples "FIP to FIP with same provider", scenario
      end
    end
  end
end
