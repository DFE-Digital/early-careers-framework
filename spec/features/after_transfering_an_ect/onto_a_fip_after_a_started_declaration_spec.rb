...
RSpec.feature "after Transferring the only ECT from another school onto a FIP", type: :feature do
  ...
  context "from a FIP with same provider, after a started declaration has occurred" do
    before do
    ...
    scenario "the declarations cannot be seen by another Lead Provider - INCORRECT" do
      then_participant_declarations_cannot_be_seen_by_lead_provider @lead_providers[1],
                                                                    @participants.first
    end
  ...
  context "from a FIP with a different provider, after a started declaration has occurred" do
    before do
      given_lead_provider_contracted_to_deliver_ecf
      and_another_lead_provider_contracted_to_deliver_ecf
      and_another_lead_provider_contracted_to_deliver_ecf
    ...
    scenario "the declarations cannot be seen by the new Lead Provider - INCORRECT" do
      then_participant_declarations_cannot_be_seen_by_lead_provider @lead_providers[1],
                                                                    @participants.first
    end
    ...
    scenario "the declarations cannot be seen by the original Lead Provider" do
      then_participant_declarations_cannot_be_seen_by_lead_provider @lead_providers[2],
                                                                    @participants.first
    end
