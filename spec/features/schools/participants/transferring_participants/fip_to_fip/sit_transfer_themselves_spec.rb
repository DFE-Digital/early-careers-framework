...
RSpec.describe "SIT transferring themselves", type: :feature, js: true do
  before do
  ...
  scenario "Induction tutor can transfer their mentor profile to their schools programme" do
    ...
    then_i_should_be_taken_to_the_sit_mentor_check_your_answers_page

    click_on "Confirm and add"
    then_i_should_be_on_the_sit_mentor_complete_page
  ...
  scenario "Induction tutor can transfer their SIT mentor profile to their schools programme" do
    given_i_am_also_the_sit_where_i_mentor
  ...
  scenario "Induction tutor can transfer their mentor profile and continue their current programme" do
    when_i_click_to_add_a_new_ect_or_mentor
    then_i_should_be_on_the_who_to_add_page
  ...
  def given_i_am_also_the_sit_where_i_mentor
    @induction_coordinator_profile_2 = create(:induction_coordinator_profile, schools: [@school_cohort_two.school], user: @mentor.user)
  end
  ...
  def when_i_update_the_email_with_the_sit_email
    when_i_update_the_email_with(@induction_coordinator_profile.user.email)
  end
  ...
  def then_i_should_be_taken_to_the_teachers_current_programme_page
    expect(page).to have_selector("h1", text: "Will #{@induction_coordinator_profile.user.full_name} continue with their current training programme?")
  ...
  def then_i_should_be_taken_to_the_schools_current_programme_page
    expect(page).to have_selector("h1", text: "Will #{@induction_coordinator_profile.user.full_name} be training with your school’s current providers?")
  ...
  def then_i_should_be_taken_to_the_sit_mentor_check_your_answers_page
    expect(page).to have_selector("h1", text: "Check your answers")
    expect(page).to have_selector("dd", text: @induction_coordinator_profile.user.full_name)
    expect(page).to have_selector("dd", text: @participant_data[:trn])
  end
