...
RSpec.shared_context "ECT has been withdrawn" do
  before do
...
RSpec.describe "transferring participants", :with_default_schedules, type: :feature, js: true, rutabaga: false do
  context "Attempting to transfer an ECT to a school", with_feature_flags: { cohortless_dashboard: "inactive" } do
    include_context "ECT has been withdrawn"
  end

  context "Attempting to transfer an ECT to a school cohortlessly", with_feature_flags: { cohortless_dashboard: "active" } do
    include_context "ECT has been withdrawn"
  end
end
