...
RSpec.describe "transferring participants", with_feature_flags: { change_of_circumstances: "active" }, type: :feature, js: true, rutabaga: false do
  context "Transferring a Mentor to a school" do
    context "Mentor has matching lead provider only" do
      before do
      ...
      scenario "Induction tutor can transfer an Mentor to their school" do
        ...
        then_i_should_be_taken_to_the_choose_delivery_partner_page
        when_i_select(@delivery_partner.name)
        click_on "Continue"
      ...
      def then_i_should_be_taken_to_the_choose_delivery_partner_page
        expect(page).to have_selector("h1", text: "Choose which delivery partner #{@participant_data[:full_name]} will train with")
