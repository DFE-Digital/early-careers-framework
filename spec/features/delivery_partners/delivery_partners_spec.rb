...
RSpec.feature "Delivery partner choose organisations page", type: :feature do
  ...
  def given_user_with_one_delivery_partner_exists
    @delivery_partner_user = create(:user)
    @delivery_partner_profile1 = create(:delivery_partner_profile, user: @delivery_partner_user)
  end

  def given_user_with_multiple_delivery_partner_exists
    @delivery_partner_user = create(:user)
    @delivery_partner_profile1 = create(:delivery_partner_profile, user: @delivery_partner_user)
    @delivery_partner_profile2 = create(:delivery_partner_profile, user: @delivery_partner_user)
    @delivery_partner_profile3 = create(:delivery_partner_profile, user: @delivery_partner_user)
  end
  ...
  def and_i_see_multiple_delivery_partners
    expect(page).to have_content(@delivery_partner_profile1.delivery_partner.name)
    expect(page).to have_content(@delivery_partner_profile2.delivery_partner.name)
    expect(page).to have_content(@delivery_partner_profile3.delivery_partner.name)
  end
