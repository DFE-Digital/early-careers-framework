...
module "application_configuration" {
  ...
  secret_variables = {
    ...
    ANALYTICS_DATABASE_URL = "postgres://${urlencode(module.postgres.username)}:${urlencode(module.postgres.password)}@${module.postgres.host}:${module.postgres.port}/${azurerm_postgresql_flexible_server_database.analytics[0].name}"
  }
}
