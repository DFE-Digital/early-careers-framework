SELECT
    "training_record_states"."participant_profile_id",
    ...
    CASE
        WHEN "training_record_states"."training_state" = ANY(ARRAY['withdrawn_programme', 'withdrawn_training', 'deferred_training', 'completed_training', 'leaving', 'left'])
            THEN "training_record_states"."training_state"
        WHEN "training_record_states"."validation_state" <> 'valid'
            THEN "training_record_states"."validation_state"
        WHEN NOT ("training_record_states"."training_eligibility_state" = ANY(ARRAY['eligible_for_mentor_training', 'eligible_for_mentor_training_no_partner', 'eligible_for_induction_training', 'eligible_for_induction_training_no_partner', 'not_yet_mentoring']))
            THEN "training_record_states"."training_eligibility_state"
        WHEN "training_record_states"."training_programme" = 'full_induction_programme' AND NOT ("training_record_states"."fip_funding_eligibility_state" = ANY(ARRAY['eligible_for_mentor_funding', 'eligible_for_mentor_funding_primary', 'eligible_for_fip_funding']))
            THEN "training_record_states"."fip_funding_eligibility_state"
        ELSE
            "training_record_states"."training_state"
        END as "record_state"
...
FROM (
     SELECT
         ...
         CASE
             ...
             WHEN "teacher_profiles"."trn" IS NULL AND "ecf_participant_validation_data" IS NULL
                 ...
                 CASE
                     ...
                     WHEN "emails"."status" = ANY(ARRAY['permanent-failure', 'technical-failure', 'temporary-failure'])
         ...
         CASE
             ...
             WHEN "participant_profiles"."type" = 'ParticipantProfile::Mentor'
                 THEN
                 CASE
                     WHEN "mentees"."total" > 0 AND "induction_programmes"."training_programme" = 'full_induction_programme' AND "partnerships"."lead_provider_id" IS NULL
                         THEN 'eligible_for_mentor_training_no_partner'
                     WHEN "mentees"."total" > 0
             ...
             WHEN ("participant_profiles"."type" = 'ParticipantProfile::Mentor' AND "ecf_participant_validation_data"."trn" IS NOT NULL AND "teacher_profiles"."trn" IS NULL) OR ("participant_profiles"."type" = 'ParticipantProfile::ECT' AND "teacher_profiles"."trn" IS NULL)
                 THEN 'tra_record_not_found'
             WHEN "induction_programmes"."training_programme" = 'full_induction_programme' AND "partnerships"."lead_provider_id" IS NULL
                 THEN 'eligible_for_induction_training_no_partner'
             ELSE
         ...
         CASE
             ...
             WHEN "participant_profiles"."type" = 'ParticipantProfile::Mentor'
                 ...
                 CASE
                     WHEN "induction_programmes"."training_programme" = 'full_induction_programme'
                         THEN
                         CASE
                             WHEN "mentees"."total" > 0 AND "partnerships"."lead_provider_id" IS NULL
                                 THEN 'active_fip_mentoring_no_partner'
                             WHEN "mentees"."total" > 0 AND "ecf_participant_eligibilities"."reason" = 'previous_participation'
                                 THEN 'active_fip_mentoring_ero'
                             WHEN "mentees"."total" > 0
                                 THEN 'active_fip_mentoring'
                             WHEN "partnerships"."lead_provider_id" IS NULL
                                 THEN 'not_yet_mentoring_fip_no_partner'
                             WHEN "ecf_participant_eligibilities"."reason" = 'previous_participation'
                                 THEN 'not_yet_mentoring_fip_ero'
                             ELSE
                     ...
                     ELSE
                         CASE
                             ...
                             WHEN "mentees"."total" > 0
                                 THEN 'active_cip_mentoring'
                             WHEN "ecf_participant_eligibilities"."reason" = 'previous_participation'
                                 THEN 'not_yet_mentoring_cip_ero'
                             ELSE
             ...
             ELSE
                 CASE
                     ...
                     WHEN "induction_programmes"."training_programme" = 'full_induction_programme' AND "ecf_participant_eligibilities"."status" = 'manual_check' AND "ecf_participant_eligibilities"."reason" = 'no_induction'
                         THEN 'registered_for_fip_training'
                     WHEN "induction_programmes"."training_programme" = 'full_induction_programme'
                         THEN 'active_fip_training'
                     WHEN "induction_programmes"."training_programme" = 'core_induction_programme' AND "ecf_participant_eligibilities"."status" = 'manual_check' AND "ecf_participant_eligibilities"."reason" = 'no_induction'
                         THEN 'registered_for_cip_training'
     ...
     FROM "participant_profiles"
              ...
              LEFT OUTER JOIN "appropriate_bodies"
                              ON "appropriate_bodies"."id" = "induction_records"."appropriate_body_id"
              ...
              LEFT OUTER JOIN (SELECT "induction_records"."mentor_profile_id", count(*) as total FROM "induction_records" GROUP BY "induction_records"."mentor_profile_id", "induction_records"."participant_profile_id") as "mentees"
                              ON "mentees"."mentor_profile_id" = "participant_profiles"."id"
) as "training_record_states"
GROUP BY
    "training_record_states"."participant_profile_id",
    "training_record_states"."induction_record_id",
    "training_record_states"."school_id",
