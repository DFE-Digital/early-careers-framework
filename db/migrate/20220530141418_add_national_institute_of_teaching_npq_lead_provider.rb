...
class AddNationalInstituteOfTeachingNPQLeadProvider < ActiveRecord::Migration[6.1]
  def change
    ...
    npq_lead_provider = NPQLeadProvider.find_or_create_by!(name: niot_name, id: niot_id, vat_chargeable: vat_chargeable)
    cpd_lead_provider = CpdLeadProvider.find_or_create_by!(name: niot_name)
    npq_lead_provider.update!(cpd_lead_provider: cpd_lead_provider)
  end
end
