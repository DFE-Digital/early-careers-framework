...
Providers can now make calls to API 3.0.0.0. in the production environment.
...
* an `id` parameter has been added to the [endpoint specifications for ECF schools](/api-reference/reference-v3.html#api-v3-schools-ecf-get)
...
* If providers do integrate with v3, providers must coordinate with the DfE to ensure historical records have been synced and to manage load on the service.
