...
module FinanceHelper
  ...
  def total_payment(breakdown)
    service_fee = Array.wrap(breakdown[:service_fees]).map { |params| params[:monthly] }.inject(&:+)
    output_payment = Array.wrap(breakdown[:output_payments]).map { |params| params[:subtotal] }.inject(&:+)
    other_fees = breakdown[:other_fees] ? breakdown[:other_fees].values.map { |other_fee| other_fee[:subtotal] }.inject(&:+) : 0
