...
module FinanceHelper
  ...
  def total_payment_combined(breakdown_started:, breakdown_retained_1:, ecf_lead_provider:)
    ((total_payment(breakdown_started) + total_vat(breakdown_started, ecf_lead_provider)) + (total_payment(breakdown_retained_1) + total_vat(breakdown_retained_1, ecf_lead_provider)))
  end

  def total_payment_with_vat_combined(breakdown_started:, breakdown_retained_1:, lead_provider:)
    (total_payment_with_vat(breakdown_started, lead_provider) + total_payment_with_vat(breakdown_retained_1, lead_provider))
  end

  def total_vat_combined(breakdown_started:, breakdown_retained_1:, lead_provider:)
    total_vat(breakdown_started, lead_provider) + total_vat(breakdown_retained_1, lead_provider)
  end
