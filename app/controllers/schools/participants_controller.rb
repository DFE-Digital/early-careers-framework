...
class Schools::ParticipantsController < Schools::BaseController
  ...
  def add_appropriate_body
    if (FeatureFlag.active?(:cohortless_dashboard) && @profile.ect?) || (!FeatureFlag.active?(:cohortless_dashboard) && can_appropriate_body_be_changed?)
