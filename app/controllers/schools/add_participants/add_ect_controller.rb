...
module Schools
  module AddParticipants
    class AddECTController < BaseController
      ...
      def update
        if @form.valid?
          ...
          # redirect_to show_schools_add_ect_participants_path(cohort_id: @school_cohort.cohort.start_year, step: @wizard.next_step_path)
        else
          render @wizard.current_step
        end
      end
      ...
      def save_appropriate_body
        ...
        redirect_to show_schools_add_ect_participants_path(cohort_id: @school_cohort.cohort.start_year, step: "check-answers")
      end
    ...
    private
      ...
      def initialize_wizard
        if request.get?
          @wizard = AddECTWizard.new(current_step: step_name,
                                     current_state:,
                                     current_user:,
      ...
      def wizard_back_link_path
        show_schools_add_ect_participants_path(cohort_id: @school_cohort.cohort.start_year, step: @wizard.previous_step_path)
      end

      def current_state
        session[:add_ect_wizard] ||= {}
      end
      ...
      def submitted_params
        params.fetch(:add_ect_wizard, {}).permit(AddECTWizard.permitted_params_for(step_name))
      end
