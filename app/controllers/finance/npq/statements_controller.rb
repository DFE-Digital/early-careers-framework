  ...
  module NPQ
    class StatementsController < BaseController
      def show
        ...
        @npq_contracts_for_information = @npq_lead_provider.npq_contracts.where(
          version: @statement.contract_version,
          cohort: @statement.cohort,
        ).order(course_identifier: :asc)

        @npq_contracts = @npq_contracts_for_information.where(special_course: false)
        @npq_special_contracts = @npq_contracts_for_information.where(special_course: true)
