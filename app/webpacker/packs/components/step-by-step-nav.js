...
if (stepByStep) {
  ...
  function rememberStepState(step) {
    if (stepByStep.rememberShownStep) {
      const data =
        JSON.parse(loadFromSessionStorage(stepByStep.uniqueId)) || [];
  ...
  function ensureOnlyOneActiveLink() {
    ...
    if (checkLink) {
      if (
        !checkLink.parentNode.classList.contains(stepByStep.activeLinkClass)
      ) {
  ...
  class StepView {
    constructor(stepElement, $module) {
      ...
      this.titleButton = stepElement.querySelectorAll(
        ".js-step-title-button"
      )[0];
      const textElement = stepElement.querySelectorAll(
        ".js-step-title-text"
      )[0];
