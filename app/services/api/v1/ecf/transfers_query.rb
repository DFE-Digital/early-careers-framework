      ...
      class TransfersQuery
        ...
        def induction_records
          join = InductionRecord
                   .select("DISTINCT FIRST_VALUE(induction_records.id) OVER (#{latest_induction_record_order}) AS latest_id, induction_records.participant_profile_id as participant_profile_id")
          ...
          profiles = ParticipantProfile
            .distinct
            .joins(:induction_records, participant_identity: :user)
            .includes(
              :user,
              :participant_identity,
              induction_records: [:schedule,  induction_programme: [school_cohort: :school, partnership: :lead_provider]]
            ).joins("JOIN (#{join.to_sql}) AS latest_induction_records ON latest_induction_records.participant_profile_id = participant_profiles.id")
            .where(induction_records: { induction_status: "leaving" })
          ...
          #           SQL
          # ids = InductionRecord.group(:participant_profile_id).having("count(Distinct school_cohorts.school_id) > 1").joins(:participant_profile, :schedule, { induction_programme: [:partnership, :school_cohort] }).count.map {|f,v| f }
                   # .where(
                   #   schedule: { cohort_id: with_cohorts.map(&:id) },
                   #   induction_programme: {
                   #     partnerships: {
                   ...
                   #   },
                   # )
          # ids = InductionRecord.group(:participant_profile_id).where(induction_status: "leaving").count.map {|f,v| f }
        end
