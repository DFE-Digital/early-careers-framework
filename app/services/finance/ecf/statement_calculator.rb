...
module Finance
  module ECF
    class StatementCalculator
      ...
      def started_count
        output_calculator.band_letters.sum do |letter|
          output_calculator.banding_for("started").additions(letter)
        end
      end

      def retained_types
        event_types.select { |t| t.to_s.starts_with?("retained_") }
      end

      def retained_count
        output_calculator.band_letters.sum do |letter|
          retained_types.sum do |event_type|
            output_calculator.banding_for(event_type.to_s.dasherize)&.additions(letter) || 0
          end
        end
      end
      ...
      def completed_count
        output_calculator.band_letters.sum do |letter|
          output_calculator.banding_for("completed").additions(letter)
        end
      end

      def extended_types
        event_types.select { |t| t.to_s.starts_with?("extended_") }
      end
