...
class ValidationBetaService
  ...
  def notify_induction_coordinators_about_validation
    School.eligible.find_each do |school|
      if added_participants_and_not_in_beta(school) || chosen_programme_but_not_added_participants(school)
...
private
  ...
  def added_participants_and_not_in_beta(school)
    !FeatureFlag.active?(:participant_validation, for: school) && school.ecf_participants.present?
  end

  def chosen_programme_but_not_added_participants(school)
    school.chosen_programme?(Cohort.current) && school.ecf_participants.blank?
  end
