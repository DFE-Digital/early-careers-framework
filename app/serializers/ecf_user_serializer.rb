...
class ECFUserSerializer
  ...
  attributes :user_type do |user|
    case user.teacher_profile.ecf_profiles.first&.type
  ...
  attributes :registration_completed do |user|
    user.teacher_profile.ecf_profiles.first&.completed_validation_wizard?
  end
  ...
  def self.find_school_cohort(user)
    user.teacher_profile.ecf_profiles.first&.school_cohort
  end

  def self.find_core_induction_programme(user)
    user.teacher_profile.ecf_profiles.first&.core_induction_programme ||
